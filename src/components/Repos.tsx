...
const Repos: FC<{
  ...
  isLoading: boolean
  repositories: Repo[]
}> = ({ onlyFive = true, isLoading, repositories }) => {
  ...
  const repos = repositories
    ...
    .filter((i) => i.language?.includes(selectValue))
  ...
  const url = location.search?.split('=')[1]?.length
    ? '/repositories?search=' + location.search?.split('=')[1]
    : '/repositories'
    ...
    <>
      <div className="flex flex-col">
        ...
        <ul className="flex flex-wrap justify-between gap-y-4 mt-4">
          {repos
            ?.slice(0, onlyFive ? 5 : repos.length)
            .map((repo, index) => <Repository repository={repo} key={index} />)}
        </ul>
        <div
