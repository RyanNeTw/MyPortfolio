        ...
        <div>
          <h3 className="text-sec font-mono font-semibold transition group-hover:text-pri">
            {repository.name[0].toUpperCase() +
              repository.name.replaceAll('-', ' ')?.trim()?.slice(1)}
          </h3>
          {repository.description && (
            <Paragraphe text={repository.description} size={true} />
          )}
        </div>
        <div className="w-full flex flex-col items-start">
          {repository.language && <Badge text={`${repository.language}`} />}
          <div
            className={`flex flex-row justify-between items-end w-full ${repository.stargazers_count === 0 && 'mt-2'}`}
          >
            <h4 className="text-sec text-xs">
              {getDate(repository.created_at)}
            </h4>
