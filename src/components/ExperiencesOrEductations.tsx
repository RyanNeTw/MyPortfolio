...
}> = ({ object, subTitle, accordionIsOpen }) => {
  return (
    <>
      <Accordion
        subTitle={subTitle}
      ...
      <a href={object.website} target="_blank" rel="noreferrer">
        <div className="flex flex-col gap-2 hover:scale-105 transition group">
          <div className="flex flex-row gap-2">
            ...
            />
            <div>
              <h5 className="font-mono text-xs text-sec group-hover:text-pri transition">
                {object.place}
              </h5>
          ...
          <h5 className="font-mono text-xs text-sec self-end">
            {getDate(object.date_start)}{' '}
            {object?.date_end && `to ${getDate(object?.date_end)}`}
          </h5>
        </div>
      </a>
    </>
  ...
  return (
    <>
      <ul className="flex flex-col gap-4">
        {object?.map((o, index) => <Element object={o} key={index} />)}
      </ul>
    </>
  )
}
